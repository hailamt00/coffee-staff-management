...
# Mapping provided by psql SELECT id, name, phone FROM employees;
id_map = {
    "Thanh Tú": 70,
    "Trịnh Hải Lâm": 71,
    "Phương Ly": 72,
...
with open("d:\\GitHub\\coffee-staff-management\\import_attendance.sql", "w", encoding="utf-8") as f:
    f.write("-- Attendance Data Import\n")
    f.write("BEGIN;\n")
    f.write("TRUNCATE TABLE attendance CASCADE;\n")
    
    lines = raw_data.strip().split("\n")
    for line in lines:
        # User provided tabbed columns, let's use a regex to handle possible variations
        ...
        name = parts[2].strip()
        phone = parts[3].strip()
        pos_text = parts[4].strip()
        check_in_time = parts[5].strip()
        
        # Check out time is in column index 6
        check_out_time = ""
        if len(parts) > 6:
            check_out_time = parts[6].strip()
            
        # Total hours is in column index 7
        diff = ""
        if len(parts) > 7:
            diff = parts[7].strip()
            
        # Note is in column index 8
        note = ""
        if len(parts) > 8:
            note = parts[8].strip()
        ...
        if not emp_id:
            print(f"DEBUG: Employee not found: {name}")
            continue
            
        # Combine date and time for timestamps
        check_in_ts = f"'{date} {check_in_time}:00'"
        
        if check_out_time and check_out_time != "":
            check_out_ts = f"'{date} {check_out_time}:00'"
        else:
            check_out_ts = "NULL"
            
        diff_val = diff if diff and diff != "" else "NULL"
        note_val = f"'{note}'" if note else "NULL"
        
        sql = f"INSERT INTO attendance (employee_id, check_in, check_out, total_hours, note) VALUES ({emp_id}, {check_in_ts}, {check_out_ts}, {diff_val}, {note_val});\n"
        f.write(sql)
        
    f.write("COMMIT;\n")

print("SUCCESS: SQL script generated at d:\\GitHub\\coffee-staff-management\\import_attendance.sql")
