    ...
    return (
        <div className="space-y-6 pb-20">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between px-2 gap-6">
                ...
                <div className="flex items-center gap-2 bg-slate-100 dark:bg-neutral-800 p-1 rounded-lg self-start sm:self-auto">
                    <Button
                        variant="ghost"
                        size="sm"
                    ...
                    <div className="px-2 text-[11px] sm:text-sm font-bold min-w-[120px] sm:min-w-[140px] text-center">
                        {(() => {
                            const curr = new Date(date)
                            const day = curr.getDay()
                            const diff = curr.getDate() - day + (day === 0 ? -6 : 1)
                            ...
                            <div className="space-y-2 pt-2">
                                ...
                                {reqPositionId ? (
                                    <div className="space-y-4">
                                        ...
                                        <div className="hidden sm:block border border-slate-200 dark:border-neutral-800 rounded-xl overflow-hidden overflow-x-auto shadow-sm">
                                            <Table>
                                                    ...
                                                    <TableRow>
                                                        ...
                                                        {weekDates.map(d => (
                                                            <TableHead key={d.toISOString()} className="text-[10px] text-center p-2 border-r border-slate-100 dark:border-neutral-800 last:border-r-0">
                                                                <div className="font-bold text-slate-800 dark:text-slate-200">{d.toLocaleDateString('en-US', { weekday: 'short' })}</div>
                                                                <div className="text-slate-400 font-medium">{d.toLocaleDateString('en-US', { day: '2-digit', month: '2-digit' })}</div>
                                                            </TableHead>
                                                        ))}
                                                    </TableRow>
                                                </TableHeader>
                                                                            ...
                                                                            <div className="flex justify-center">
                                                                                <Checkbox
                                                                                    ...
                                                                                    onCheckedChange={(checked) => {
                                                                                        if (editMode) {
                                                                                            if (checked) setSelectedCells([key])
                                        ...
                                        <div className="sm:hidden space-y-3 max-h-[400px] overflow-y-auto px-1 custom-scrollbar">
                                            {weekDates.map(d => {
                                                const dateStr = d.toISOString().slice(0, 10)
                                                return (
                                                    <div key={dateStr} className="space-y-2">
                                                        <div className="text-[10px] font-black text-slate-400 uppercase tracking-widest pl-1">{d.toLocaleDateString('en-US', { weekday: 'short', day: '2-digit', month: '2-digit' })}</div>
                                                                ...
                                                                return (
                                                                    <div
                                                                        ...
                                                                        onClick={() => {
                                                                            ...
                                                                            if (editMode) {
                                                                                setSelectedCells(isSelected ? [] : [key])
                                                                            } else {
                                                                                setSelectedCells(prev => isSelected ? prev.filter(k => k !== key) : [...prev, key])
                                                                            }
                                                                        }}
                                                                        className={`
                                                                            ...
                                                                            ${isSelected
                                                                                ...
                                                                                : isDisabled
                                                                                    ? 'bg-slate-50 dark:bg-neutral-800 opacity-40 cursor-not-allowed border-slate-100 dark:border-neutral-800'
                                                                                    : 'bg-white dark:bg-neutral-900 border-slate-200 dark:border-neutral-800'}
                                                                        `}
                                                                    >
                                                                        <span className={`text-[9px] font-black uppercase tracking-tight ${isSelected ? 'text-blue-600' : 'text-slate-50'}`}>{s.name}</span>
                                                                        <span className="text-[8px] font-bold text-slate-400">{(s.startTime || '').slice(0, 5)}</span>
                                                                        {isSelected && <div className="absolute top-1 right-1 w-1.5 h-1.5 rounded-full bg-blue-500" />}
                                                                        {isAssigned && <div className="absolute top-1 right-1 w-1.5 h-1.5 rounded-full bg-emerald-500" title="Assigned" />}
                                                                    </div>
                                                                )
                                                            })}
                                                        </div>
            ...
            <div className="mx-2">
                ...
                <div className="hidden md:block overflow-hidden border border-slate-200/60 dark:border-neutral-800/60 bg-white/80 dark:bg-neutral-900/80 backdrop-blur-md shadow-sm rounded-2xl overflow-x-auto">
                    <Table>
                        <TableHeader>
                        ...
                        <TableBody>
                            {Array.from(new Set(allShifts.map(s => s.name))).map((shiftName) => (
                                <TableRow key={shiftName} className={`hover:bg-slate-50/30 dark:hover:bg-white/5 transition-colors border-b border-slate-50 dark:border-neutral-800/50 last:border-0`}>
                                    ...
                                    {weekDates.map(d => {
                                        const dateStr = d.toISOString().slice(0, 10)
                                        const item = myScheduleMap.get(`${shiftName}-${dateStr}`)

                                        const statusStyles = item?.type === 'official'
                                            ? 'bg-slate-900 dark:bg-white text-white dark:text-black shadow-lg opacity-100'
                                            : item?.status?.toLowerCase() === 'pending'
                                                ? 'bg-white dark:bg-neutral-900 text-slate-900 dark:text-white border-2 border-dashed border-slate-200 dark:border-neutral-800 opacity-100'
                                                : 'opacity-10'
                                            ...
                                            >
                                                {item ? (
                                                    ...
                                                    <div className="flex flex-col items-center justify-center h-full group">
                                                        <div className="w-1.5 h-1.5 rounded-full bg-slate-300 dark:bg-neutral-600 group-hover:scale-150 group-hover:bg-black dark:group-hover:bg-white transition-all" />
                                                    </div>
                                                )}
                                            </TableCell>
                                        )
                ...
                <div className="md:hidden space-y-4">
                    {weekDates.map(d => {
                        const dateStr = d.toISOString().slice(0, 10)
                        const dailyShifts = Array.from(new Set(allShifts.map(s => s.name))).map(sn => ({
                            name: sn,
                            item: myScheduleMap.get(`${sn}-${dateStr}`)
                        }))
                        const hasActivity = dailyShifts.some(s => s.item)

                        return (
                            <div key={dateStr} className="bg-white/80 dark:bg-neutral-900/80 rounded-2xl border border-slate-200/60 dark:border-neutral-800/60 p-4 shadow-sm overflow-hidden">
                                <div className="flex justify-between items-center mb-4 border-b border-slate-50 dark:border-neutral-800 pb-3">
                                ...
                                <div className="grid grid-cols-1 gap-3">
                                    {dailyShifts.map(shift => {
                                        ...
                                        return (
                                            <div
                                                ...
                                                onClick={() => handleCellClick(item, dateStr)}
                                                className={`
                                                    relative p-4 rounded-xl border flex items-center justify-between cursor-pointer transition-all active:scale-[0.98]
                                                    ${isOfficial
                                            ...
                                            >
                                                <div className="flex flex-col gap-1">
                                                    <span className={`text-[10px] font-black uppercase tracking-wider ${isOfficial ? 'text-slate-400 dark:text-slate-500' : 'text-slate-400'}`}>
                                                        {name}
                                                    </span>
                                                    <div className="flex items-center gap-2">
                                                        <Clock size={12} className={isOfficial ? 'text-white dark:text-black' : 'text-slate-600 dark:text-slate-400'} />
                                                        <span className={`text-sm font-bold ${isOfficial ? 'text-white dark:text-black' : 'text-slate-900 dark:text-white'}`}>
                                                            {(item.shiftStartTime || item.startTime)?.slice(0, 5)}
                                                        </span>
                                                    </div>
                                                </div>
                                    ...
                                    {/* Quick Request Button for this specific day if no active shift for a slot */}
                                    <Button
                                        variant="ghost"
